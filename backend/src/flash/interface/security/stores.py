"""Ports de stockage pour la sécurité + implémentations Redis.
...
- ``AccessRevocationStore`` : liste de révocation des ``jti`` d'*access token* (déconnexion
  immédiate), avec TTL = durée de vie restante du jeton.
- ``RateLimiter`` : compteur à fenêtre fixe ; ``hit`` renvoie ``False`` quand le quota est
  dépassé.
"""
