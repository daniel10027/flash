...
> **Phase 1 terminée + Phase 2 : BE-025 → BE-032, BE-034/035/036, BE-038**
> (auth, numéros, wallets, transfert, **demandes de paiement**, **dépôt & retrait cash
> agent**, relevé, **KYC**).
...
> **27 chemins** : `auth` (6), `phones` (5), `wallets` (2), `transfers` (1),
> `payment-requests` (4), `statement` (1), `withdrawals` (2), `agent` (2), `kyc` (4),
> `admin/kyc` (1) + `/health*`, `/openapi.json`, `/docs`, `/redoc`.
> **Transfert + cash + KYC + demandes de paiement vérifiés end-to-end via docker compose** :
> dépôt agent 200 000 → wallet crédité, float 1 000 000 − 200 000 + 2 000 = 802 000 ;
> retrait 50 000 → règlement (solde 150 000), float 852 500 ; **ledger équilibré** ;
> KYC tier 1 → revue clé admin → `KycTier` 0 → 1 ; demande 20 000 → acceptation → payeur
> débité 20 160 (frais 0,8 %), demandeur crédité 20 000, `resulting_transfer_id` fixé,
> ré-acceptation sans double débit.
> 532 tests unit + 9 d'intégration (Postgres réel), couverture 100 % domain+application,
> ruff + mypy stricts.
> **Prochaine : BE-033 (paiement marchand QR), BE-037 (annulation/remboursement),
> BE-039 (reçu détaillé), BE-041/042 (notifications SSE).**
...
| Backend (BE) | [docs/tasks/backend.md](docs/tasks/backend.md) | 36 / 78 |
...
## Phase 2 — Cas d'usage cœur (en cours : 12 / 22)
...
`BE-033` — Paiement marchand par QR : `PresentMerchantQr` (statique = id marchand ;
dynamique = montant + référence + expiration) / `PayMerchant`. Frais marchand selon
`pricing_rules`, règlement marchand différé via le compte `bank_settlement`. Puis
`BE-037` (annulation / remboursement via `LedgerTransaction.reversal`), `BE-039` (reçu
détaillé), `BE-041/042` (notifications SSE in-app). ✅ Livrés : `BE-029` (KYC),
`BE-032` (demandes de paiement), `BE-034` → `BE-036` (cash agent).
